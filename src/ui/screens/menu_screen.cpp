...
void MenuScreen::create(BluetoothManager *bluetooth, GrindController *grind_ctrl, GrindingScreen *grind_screen, class HardwareManager *hw_mgr, DiagnosticsController *diag_ctrl)
{
  ...
  grind_size_increase_button = nullptr;
  grind_size_decrease_button = nullptr;
  grind_size_reset_button = nullptr;
  //grind_size_motor_button = nullptr;
...
void MenuScreen::create_grind_size_page(lv_obj_t *parent)
{
  ...
  lv_label_set_text(grind_size_label, "0 °");
  lv_obj_set_size(grind_size_label, LV_PCT(100), LV_SIZE_CONTENT);
  ...
  grind_size_decrease_button = create_button(parent, "-", lv_color_hex(THEME_COLOR_ACCENT), 102, 80, &lv_font_montserrat_28);
  grind_size_increase_button = create_button(parent, "+", lv_color_hex(THEME_COLOR_ACCENT), 102, 80, &lv_font_montserrat_28);
  grind_size_reset_button = create_button(parent, "Reset", lv_color_hex(THEME_COLOR_WARNING), 240, 80, &lv_font_montserrat_28);
  ...
  if (grind_size_reset_button)
  {
    lv_obj_add_event_cb(grind_size_reset_button, EventBridgeLVGL::dispatch_event, LV_EVENT_CLICKED,
                        reinterpret_cast<void *>(static_cast<intptr_t>(ET::MENU_GRIND_SIZE_RESET)));
  }
}
...
{
  if (!hardware_manager || !brightness_normal_slider || !brightness_screensaver_slider)
    return;
...
void MenuScreen::update_grind_size_label(float rotation)
{
  if (!grind_size_label)
  {
    return;
  ...
  char buffer[32];
  snprintf(buffer, sizeof(buffer), "%.01f °", rotation);
  lv_label_set_text(grind_size_label, buffer);
}
...
void MenuScreen::update_grinder_purge_amount_label(float amount_g)
{
  if (grinder_purge_amount_label)
  {
    ...
    if (clamped_amount < GRIND_PURGE_AMOUNT_MIN_G)
      clamped_amount = GRIND_PURGE_AMOUNT_MIN_G;
    if (clamped_amount > GRIND_PURGE_AMOUNT_MAX_G)
      clamped_amount = GRIND_PURGE_AMOUNT_MAX_G;
